...
Claude Code is a handy coding agent that runs right in your terminal. By default it uses Anthropic models through Anthropic's own API. But you can point it at a custom provider like 9router instead. 9router is a local model router that sits between your tools and different LLM providers, and I covered it in detail in [this guide](/blogs/setting-up-9router-in-pi-dev).

A custom provider is any service that serves Anthropic-compatible models through its own endpoint. Using one gives you more model options, often at a lower cost, without changing how you work in the terminal.

The whole setup is just a few lines in one JSON file. This guide walks through it step by step.
...
Fun fact: the hardest part of this step is usually remembering where you put the comma. JSON is a strict teacher, but it only punishes you with an error message.
