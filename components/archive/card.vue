  ...
  <UCard class="group" :ui="{ body: 'p-0 sm:p-0' }">
    <NuxtLink :to="path" class="grid grid-cols-1 sm:grid-cols-10 gap-1">
      <div class="sm:col-span-3">
        <NuxtImg
          class="h-full w-full object-cover object-center rounded sm:rounded sm:rounded shadow-lg group-hover:scale-[1.02] transition-all duration-500"
      ...
      <div class="sm:col-span-7 p-5">
        ...
        <div class="text-black dark:text-zinc-300 text-sm mt-2 mb-1 md:flex md:space-x-6">
          ...
          <div class="flex items-center gap-1 flex-wrap">
            <LogoTag />
            <p v-for="tag in tags" :key="tag" class="bg-gray-200 dark:bg-slate-900 rounded px-2 py-1 font-semibold">
              {{ tag }}
            </p>
          </div>
        </div>
